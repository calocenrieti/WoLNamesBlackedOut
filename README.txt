...
Inpaintはユーザ名を黒塗りの変わりに黒塗り周囲から中央を埋めるような処理を行います。
処理後のイメージはこちらをご確認ください → https://youtu.be/W32joBaRayg

No_Inferenceは名前の検出を行わず、SDR化、固定枠の黒塗り、動画の前後トリミングだけを行うモードです。
単純な動画編集のみを利用したい場合に利用ください。名前の検出をする時より、処理は速いです。
...
黒塗りの色変更を2度以上すると、動画処理ボタン押下直後から、画面の更新が停止してしまいます。
またinpaintなど他の設定などを変更して、再出力すると、やはり画面の更新が停止してしまいます。
その結果、動画処理は動いているものの、FPSの表示やプログレスバーは変化しません。
また停止ボタンは押せ、動画処理は止まるのですが、再度処理開始することも、×でウィンドウを閉じることもできません。（タスクバーのアイコン右クリック＞閉じる はできます。）
UIライブラリの問題でgithub issueを起票済みですが、すぐに修正はされないと思います。
当面は、黒塗りの色変えは、1度で決める、もしくは複数回、色をプレビューで試行錯誤する場合は、一度アプリを終了し、再びアプリを起動して最終的に決めた色に1度で設定して、動画処理を開始する、というような形で回避をお願いいたします。

・利用上のTIPS
検出スコアのデフォルト値は0.01としています。
あまりに誤検知が多い場合には、数値を少し大きくしてください。

街中などでもある程度消えると思いますが、あまり期待しないでください。

HDR画質の動画の場合、やや処理速度は遅くなります。（当方の環境では、SDR→SDRが25FPS、HDR→SDRが20FPS）
Inpaintは、黒塗りに比べ処理が遅いです（当方の環境では、HDR→SDR+inpaintで15FPS）
固定枠塗り箇所を増やすほど処理は遅くなります。
...
・更新履歴
20241127:
・重みを更新しました。検出精度の向上が目的です。
20241124:
・重みを更新し、ジョブアイコンなし、イニシャル設定を考慮するようになりました。
・No_Inference設定を追加しました。名前の検出を行わず、SDR化、固定消し、動画トリミングのみするモードです。
・使用ライブラリを更新しました。
20241117：
・Ultralytics Discordでアドバイスをもらい、画像縮小処理などをUltralyticsに任せるようにしました。
　これにより検出精度、処理速度が向上しています。
・上記に関連して重みファイルを更新しています。
・Inpaint機能を追加しました。黒塗り部分を周囲から補完塗りします。
・Score Thresholdの初期値を0.01に変更しました。誤検知が多い場合には数値を大きくしてください。
・ライセンスをAGPL3.0に統一しました。
20241026：
・黒塗りの重みのモデルサイズをより大きなものに変更し安定度を向上させています。
・重みモデルサイズの変更で、約3FPSほど処理が遅くなりました。
・黒塗りの重みを更新し、暗転時のチャット欄や、レディチェックやカウントダウンなどの黒塗りを強化しました。
・カラーピッカーの関数が重複していたので共通化しました。
20241015：
・黒塗りの色指示をループ処理外で実施するよう修正しました。
・プレビュー時、カラーピッカーを実行できないように修正しました。
・動画処理完了後、カラーピッカーが実行できない不具合を修正しました。
20241014：
・ユーザ名と固定枠の黒塗りの色を選択できるようにしました。
20241012：
・出力動画にcopyrightの"© SQUARE ENIX"を埋め込める設定を追加しました。
・タスクバーのアイコンを設定しました。
・入力ファイルを*.mp4限定にしました。
・動画処理中は停止以外のボタンは変更できないようにしました。
（変更しても処理に影響はありませんでしたが分かりやすくするためです）
20241010A：
