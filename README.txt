...
動画処理の後の音声処理において、コマンドプロンプトの画面が2度、一瞬表示されます。
処理を、画面非表示のサブプロセスにできていないのが原因と考えていますが、サブプロセスの理解が十分でなく、解決できていません。
見た目に気にはなりますが、実害はないので、現時点ではそのままとさせてください。
